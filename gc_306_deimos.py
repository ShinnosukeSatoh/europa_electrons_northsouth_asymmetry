...
Version
1.0.0 (Aug 1, 2022) Induced magnetic field model (Liuzzo)
...
ALFVEN_THETA = np.radians(9.94)  # ANGLE BETWEEN B0 VECTOR AND ALFVEN WING
...
Mdip = 1.55E+27         # Jupiterのダイポールモーメント 単位: A m^2
...
# 遠方しきい値(z方向) 磁気緯度で設定
z_p_rad = math.radians(12)      # 北側
z_m_rad = -math.radians(1)       # 南側
z_p = L94*math.sin(z_p_rad)
...
V0_array = np.sqrt(3*eV_array*1.602E-19/me)    # ndarray
...
def Bdip_abs(Rvec):
    ...
    # Magnetic field
    Bvec = np.array([3*z*x, 3*z*y, 2*z**2 - R2])

    Bdip_abs = A2*r_5*math.sqrt(Bvec[0]**2 + Bvec[1]**2 + Bvec[2]**2)
...
#
# %% 電場
E0 = -(L94*omgJ)*Bdip_abs(np.array([eurx, eury, eurz])+R0vec)
Ei = alp*E0
...
def mirrorpoint(lamu, alphau):
    ...
    """
    xn = math.radians(1E-5)  # 反復の開始値 [radian]
...
def comeback(RV2, req, lam0, K0):
    ...
    # 速度ベクトルの磁場に平行な成分
    vparallel = RV2[3]      # 粒子の速度

    # 速度ベクトルの磁場に垂直な成分
    vperp = 0
    if (2*K0/me + (Rho(Rvec)*omg)**2 - vparallel**2) > 0:
        vperp = math.sqrt(2*K0/me + (Rho(Rvec)*omg)**2 - vparallel**2)
    else:
        print('vperp out')

    """
    if (2*K0/me - (RV2[3]-Vdpara)**2 + (Rho(Rvec)*omg)**2) >= 0:
        vperp = math.sqrt(
            2*K0/me - (RV2[3]-Vdpara)**2 + (Rho(Rvec)*omg)**2)
    else:
        vperp = math.sqrt(2*K0/me + (Rho(Rvec)*omg)**2)
        print('vperp out')
    """

    # 磁気赤道面での速度
    # veq = math.sqrt(2*K0/me + (Rho(Rvec)*omg)**2)
    veq = math.sqrt(2*K0/me + (L94*omg)**2)
    ...
    if tau0 == 0:
        print('lam0, alphau: ', np.degrees(lam0), np.degrees(alphau))
    ...
    RV2_new[3] = - vparallel         # 磁力線に平行な速度成分 向き反転
...
def ode1(r3d, t):
    ...
    # 共回転電場
    Ecor = -np.array([
        vd[1]*Bvec[2] - vd[2]*Bvec[1],
        vd[2]*Bvec[0] - vd[0]*Bvec[2],
        vd[0]*Bvec[1] - vd[1]*Bvec[0],
    ])
    ...
    vx = A1*(Ecor[0] + r3d[4]*Bvec[2] - r3d[5]*Bvec[1])
    vy = A1*(Ecor[1] + r3d[5]*Bvec[0] - r3d[3]*Bvec[2])
    vz = A1*(Ecor[2] + r3d[3]*Bvec[1] - r3d[4]*Bvec[0])
...
def rk4_hybrid(Rinitvec, V0vec, V0, alpha_eq):
    """
    ...
    dt = FORWARD_BACKWARD*TC/(120)   # ジャイロを180分割するような時間刻み
    dt2 = dt*0.5
    ...
    # Europaの座標(初期化)
    europa = np.array([eurx, eury, eurz])
    ...
    for k in range(6000000000000000000):
        ...
        vperp2 = V1_2 - vparallel**2     # 磁力線垂直成分

        # 断熱不変量の確認
        # ve = math.sqrt(r3d[3]**2 + r3d[4]**2 + r3d[5]**2)
        mu_first_1 = vperp2/Babs(r3d[0:3]+R0vec)
        mu_first_2 = vperp2/Babs(r3d2[0:3]+R0vec)
        delmu = (mu_first_2 - mu_first_1)/mu_first_1

        if np.abs(delmu) > 0.001:
            print('delta mu: ', np.abs(delmu))
        ...
        # Europaの近傍を出た
        if r_eur > (RE+ALTITUDE+(200*gyroradius)):
            break
    ...
    if yn == 1:  # 有効な粒子
        ...
        # 保存量 K0 (運動エネルギー)
        # K0 = 0.5*me*((vparallel-Vdpara)**2 + (vperp-Vdperp)**2)
        ...
        for k in range(6000000000000000000):
            ...
            if (maglat1 < z_p_rad) and (maglat2 > z_p_rad):
                # print('UPPER')
                RV2, tau = comeback(RV, req, maglat2, K0)
                # print('tau: ', tau)
            ...
            if (((RV[2] >= 0) and (RV2[2] < 0))           # 通過方向: N → S
                ...
                if y > europa[1]-(1.06*RE):
                    ...
                    """
                    vperp = math.sqrt(2*K0/me + (Rho(Rvec2)*omgR)**2) - Vdperp
                    """

                    """
                    # こいつ時々負になる → vperp発散
                    if 2*K0/me - (RV[3]-Vdpara)**2 + (Rho(Rvec2)*omgR)**2 < 0:
                        print('OOPS!', vperp)
                    """
                    ...
                    energy_end = 0.5*me*(Vnorm**2)/float(1.602E-19)
                    """

                    energy_end = 1  # K0/float(1.602E-19)
...
def calc(gridnum, V0):
    ...
    mcolat_one = mcolatr[gridnum]
    mlong_one = mlongr[gridnum]
    ...
    i = 0
    for alpha0 in alpha_array:
        # 初期座標をシフトさせる
        mcolat, mlong = dshift(mcolat_one, mlong_one)
        ...
        # 磁気赤道面におけるピッチ角
        Beq = Babs(np.array([L94, 0, 0]))
        B0 = Babs(Rvec)
        alpha_eq = np.arcsin(np.sqrt(Beq/B0)*np.sin(alpha0))
        # print(np.degrees(alpha0), np.degrees(alpha_eq))

        # 速度ベクトル V0vec
        # alpha: 0.01 以上 179.9 未満 でランダムに与える
        # beta: 0 以上 360 未満 でランダムに与える
        # alpha = ((180-loss) - loss)*np.random.rand() + loss  # [DEGREES]
        # alpha = math.radians(alpha)                          # [RADIANS]
        alpha = alpha0
        beta = 2*np.pi*np.random.rand()                      # [RADIANS]
        V0vec = V0*np.array([
        ...
        if GYRO == 1:
            result[i, 3:9] = rk4_hybrid(
                Rinitvec, V0vec, V0, alpha_eq)    # ジャイロ→回転中心の順に
        else:
            0

        result[i, 9] = vdotn

        if result[i, 6] == 1:   # yn=1 の場合は gridnum に置き換える
            result[i, 6] = gridnum+1
...
def main():
    ...
    # グリッド番号
    gridnum = np.arange(0, np.size(mcolatr)+1, 1)

    total_s = time.time()

    for i in range(len(eV_array)):
        savename = 'gc306_'+date+'_' + \
            str(eV_array[i])+'ev_alp_'+str(alp)+'.txt'

        vabs = math.sqrt((3*eV_array[i]*1.602E-19)/me)
        ...
        args = list(zip(gridnum, V0_array[i]*np.ones(mcolatr.shape)))
        ...
        # result[:, 4] ... 終点 y座標
        # result[:, 5] ... 終点 z座標
        # result[:, 6] ... yn (grid number)
        # result[:, 7] ... 終点 energy [eV]
        # result[:, 8] ... 終点 alpha_eq [RADIANS]
        # result[:, 9] ... 出発点 v_dot_n [m/s]

        # Europaに衝突しない(yn=1)の粒子を取り出す
        yn1 = np.where(result[:, 6] != 0)  # 0でない行を見つける(検索は表面yn=6列目)
