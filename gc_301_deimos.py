...
Version
0.1.1 (Feb 24, 2022)
0.1.2 (Feb 28, 2022) -> SMTP INTRODUCED
...
def calc(mcolatr, mlongr, V0):
    ...
    """
    if np.random.rand() > 0.95:    # ときどき計算時間を表示する
        with objmode():
...
def main():
    ...
    for i in range(len(eV_array)):
        savename = 'gc301_'+date+'_'+str(eV_array[i])+'ev_alp_'+str(alp)+'.txt'
