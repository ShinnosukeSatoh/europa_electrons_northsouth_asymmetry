...
Created on Fri Feb 22 16:30:00 2022
@author: Shin Satoh
...
def calc(mcolatr, mlongr, V0):
    ...
    # result ... a trajectory
    result = rk4_hybrid(Rinitvec, V0vec, V0)    # ジャイロ→回転中心の順に
...
def main():
    ...
    savename = 'gc301_'+str(eV_array)+'ev_alp_'+str(alp)+'_'+date+'.txt'
    ...
    # 並列計算の実行
    start = time.time()
    result = calc(np.radians(colatitude),
                  np.radians(longitude), V0_array)
    stop = time.time()
    ...
    yn1 = np.where(result[:, 3] == 1)  # 0でない行を見つける(検索は表面yn=6列目)
